...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        for (int i = 0; i < count.size() - 1; i++) {
            Node left = priorityQueue.poll(); //вынимаем из очереди
            Node right = priorityQueue.poll(); //вынимаем из очереди
            Node parent = new InternalNode(left, right); //создаем родителя
            priorityQueue.add(parent); //возвращаем родителя обратно в очередь
        }
